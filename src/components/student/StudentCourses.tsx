    ...
    <div className="space-y-6">
        ...
        <Dialog open={joinDialogOpen} onOpenChange={setJoinDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDesc className="italic">
                Nhập mã lớp học để đăng ký tham gia
              </DialogDesc>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="enrollCode" className="font-semibold">
                  Mã lớp học
                </Label>
                <Input
                  id="enrollCode"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredCourses.map((course) => {
          ...
          return (
            ...
            >
              <div className="h-32 bg-gradient-to-br from-blue-600 to-blue-800 rounded-t-[calc(var(--radius)-2px)] relative overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center">
                  <BookOpen className="w-12 h-12 text-white opacity-50" />
                </div>
                <div className="absolute top-3 right-3">
                  <div className="bg-white text-primary px-2 py-1 rounded font-bold text-sm shadow-sm">
                    {course.code}
                  </div>
                </div>
              </div>
              <CardHeader className="pb-2">
                <CardTitle className="line-clamp-2 text-primary font-bold text-lg">
                  {course.name}
                </CardTitle>
      ...
      {filteredCourses.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-muted-foreground mb-2 italic">
